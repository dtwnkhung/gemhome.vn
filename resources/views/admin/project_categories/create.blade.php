...
<div class="users-table">
    <div class="row">
        <div class="col-sm-4">
            <h4 class="card-title mb-0">{{ !isset($category) ? __('app.Create :name', ['name' => __('app.Project Categories')]) : __('app.Edit :name', ['name' => __('app.Project Categories')]) }}</h4>
            <div class="small text-muted">{{ __('app.Edit category') }}
            </div>
        </div>

        <div class="col-sm-8 text-right">      
            <a class="btn btn-sm btn-primary" href="{{ route('project-categories.index') }}">
    ...
    <div class="row">
        <div class="col-sm-12">
            <form class="form-horizontal" action="{{ isset($category) ? route('project-categories.update', $category->slug) : route('project-categories.store') }}" method="POST">
                @csrf
                @if (isset($category))
                    @method('PUT')
                @endif
