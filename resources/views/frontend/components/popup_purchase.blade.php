<div class="modal fade" id="paymen_modal" tabindex="-1" role="dialog" aria-labelledby="paymen_modal_Title" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            {{ __('app.You do not have permission to access. Please register') }} <a href="{{ route('post.packages') }}">{{ __('app.here') }}</a>.
        </div>
        <div class="modal-footer justify-content-around"> 
            <a href="{{ route('post.packages') }}" type="button" class="btn btn-primary">{{ __('app.Register now') }}</a>
        </div>
    </div>
    </div>
</div>
