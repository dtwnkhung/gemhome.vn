...
class DorMikadoClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_dor_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_dor_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'dor' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'dor' );
		}
...
if ( ! function_exists( 'dor_mikado_create_like' ) ) {
	function dor_mikado_create_like() {
		DorMikadoClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'dor_mikado_create_like' );
}