...
class HeaderMinimal extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->menuAreaHeight     = dor_mikado_set_default_menu_height_for_header_types();
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['menu_area_in_grid'] = dor_mikado_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes';
		$parameters['fullscreen_menu_icon_class'] = dor_mikado_get_fullscreen_menu_icon_class();
		
		$parameters = apply_filters( 'dor_mikado_filter_header_minimal_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$menu_background_color             = dor_mikado_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = dor_mikado_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		$menu_grid_background_color        = dor_mikado_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = dor_mikado_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $menuAreaTransparent ) {
			...
			if ( ( $sliderExists && dor_mikado_is_top_bar_enabled() )
			     || dor_mikado_is_top_bar_enabled() && dor_mikado_is_top_bar_transparent()
			) {
				$transparencyHeight += dor_mikado_get_top_bar_height();
			}
		}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$menu_background_color             = dor_mikado_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = dor_mikado_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = dor_mikado_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = dor_mikado_options()->getOptionValue( 'menu_area_grid_background_transparency' );
	...
	public function calculateHeaderHeight() {
		...
		if ( dor_mikado_is_top_bar_enabled() ) {
			$headerHeight += dor_mikado_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = dor_mikado_get_meta_field_intersect( 'header_behaviour' );
		if ( ! in_array( $header_behavior, array( 'sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up' ) ) ) {
			$perPageVars['mkdfHeaderTransparencyHeight'] = $this->headerHeight - ( dor_mikado_get_top_bar_height() + $this->heightOfCompleteTransparency );
		} else {
