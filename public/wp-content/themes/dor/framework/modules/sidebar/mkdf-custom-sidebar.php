 ...
 */
if ( ! class_exists( 'DorMikadoClassSidebar' ) ) {
	class DorMikadoClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'dor' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'dor-mikado-admin-sidebar-script', MIKADO_ROOT . '/framework/admin/assets/js/mkdf-ui/mkdf-sidebar.js' );
			wp_enqueue_style( 'dor-mikado-admin-sidebar', MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'dor') . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'dor') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function delete_sidebar_area() {
			...
			if ( ! empty( $_POST['name'] ) ) {
				...
				if ( ( $key = array_search( $name, $this->sidebars ) ) !== false ) {
					...
					update_option( $this->stored, $this->sidebars );
					echo esc_html__( "sidebar-deleted", 'dor' );
				}
			}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'dor_mikado_filter_custom_widget_args', $args );
